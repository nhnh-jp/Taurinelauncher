...
import { beginMicrosoftBrowserLogin, calculateMemory, checkModUpdates, createProfile, detectJava, disableMod, enableMod, getModrinthVersions, installMod, JavaDetection, launchMinecraft, listInstalledMods, listProfiles, MemoryPlan, MicrosoftBrowserLogin, ModInfo, ModrinthSearchResult, ModUpdateResult, ProfileSummary, pollMicrosoftBrowserLogin, removeMod, searchModrinth, updateMod } from "./tauri";
...
function ModsPage({ profiles, selectedPath, onSelect }: { profiles: ProfileSummary[]; selectedPath: string; onSelect: (path: string) => void }) {
  ...
  async function applyUpdate(update: ModUpdateResult) {
    if (!selected) return;
    setBusyProject(update.file_name);
    try {
      const updated = await updateMod(selected.path, update.file_name, update.latest_version_id);
      setMessage(`Updated ${updated.file_name}`);
      await refreshMods();
      const nextUpdates = await checkModUpdates(selected.path);
      setUpdates(nextUpdates);
    } catch (error) {
...
function SettingsPage() {
  ...
  const [browserLogin, setBrowserLogin] = useState<MicrosoftBrowserLogin | null>(null);
  const [message, setMessage] = useState("");
  ...
  async function startMicrosoftLogin() {
    try {
      const login = await beginMicrosoftBrowserLogin();
      setBrowserLogin(login);
      setMessage("Browser opened. Complete Microsoft login, then return here.");
    } catch (error) {
      setMessage(error instanceof Error ? error.message : String(error));
    }
  }
  async function checkMicrosoftLogin() {
    ...
    try {
      const result = await pollMicrosoftBrowserLogin(browserLogin.state);
      setMessage(result ? "Microsoft authorization code received" : "Microsoft login is still pending");
    } catch (error) {
      setMessage(error instanceof Error ? error.message : String(error));
    }
  }
