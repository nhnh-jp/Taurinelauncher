...
pub fn calculate_memory(profile_path: String) -> Result<MemoryPlan, String> {
    ...
    let summary =
        profile_service::summarize_profile(&profile_service::resolve_profile_path(&profile_path)?)?;
    ...
    if !profile.launch.auto_memory {
        return Ok(MemoryPlan {
            total_memory_mb,
            os_reserved_mb,
            mod_count: summary.mod_count,
            recommended_mb: profile.launch.memory_max_mb as u64,
            capped_by_system: false,
    ...
    let available = total_memory_mb
        .saturating_sub(os_reserved_mb)
        .max(profile.launch.memory_min_mb as u64);
    let recommended_mb = target
        .min(available)
        .max(profile.launch.memory_min_mb as u64);
    Ok(MemoryPlan {
        total_memory_mb,
        ...
        mod_count: summary.mod_count,
        recommended_mb,
        capped_by_system: recommended_mb < target,
